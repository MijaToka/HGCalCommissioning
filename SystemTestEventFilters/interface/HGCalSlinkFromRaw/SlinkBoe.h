...
class SlinkBoe {
 public:
  ...
  inline uint8_t  boeHeader() const {
    return _word[1]>>56;
  }
  ...
  inline uint8_t  l1aSubType() const{
    return (_word[0]>>48)&0xff;
  }

  inline uint16_t l1aType() const{
  ...
  inline void setEventId(uint64_t e) {
    assert(e<(1UL<<44));
    _word[1]&=0xfffff00000000000;
    _word[1]|=e;
  }
  
  inline void setL1aSubType(uint8_t st) {
    _word[0]&=0x00ff000000000000;
    _word[0]|=uint64_t(st)<<48;
  }
  
  inline void setL1aType(uint16_t t) {
    _word[0]&=0x0000ffff00000000;
    _word[0]|=uint64_t(t)<<32;
  }
  
  inline void setSourceId(uint32_t s) {
